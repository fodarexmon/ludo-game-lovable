...
interface ProfileRow {
  ...
  country: string;
  avatar_id: string;
  stats?: { gamesPlayed: number; wins: number; totalPoints: number; coins?: number };
  isOnline?: boolean;
  lastActive?: number;
  friendCode?: string;
}
  ...
  if (err)
    return (
      <div className="min-h-screen p-6 text-center">
        <p className="mt-10 text-destructive">{err}</p>
        <Link to="/play/online" className="btn-ghost mt-4 inline-flex">
          Back
        </Link>
      </div>
    );
    ...
    return (
      <div className="min-h-screen p-6 flex flex-col relative overflow-hidden">
        ...
        <div className="mx-auto max-w-2xl w-full z-10">
          <button onClick={leave} className="btn-ghost mb-6 bg-background/50 backdrop-blur-md">
            ← Leave Room
          </button>
          ...
          <div className="panel space-y-3 bg-card/40 backdrop-blur-md border border-white/5">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm px-3 py-1 bg-black/40 rounded-full font-mono text-primary border border-primary/20">
                {players.length} / 4
              </span>
            </div>
              ...
              {[0, 1, 2, 3].map((seat) => {
                ...
                if (p && prof) {
                  ...
                  return (
                    <div
                      key={seat}
                      className="flex items-center gap-4 rounded-xl border border-white/10 bg-black/40 p-3 shadow-inner animate-in slide-in-from-right-4"
                    >
                      <div
                        className="h-10 w-10 rounded-full flex items-center justify-center shadow-lg border-2 border-black/50"
                      ...
                      <div className="flex-1">
                        ...
                        {!room.isQuickMatch && p.user_id === room.host_id && (
                        ...
                        {room.isQuickMatch && (
                          <span
                            className={`text-xs font-bold ${isReady ? "text-green-400" : "text-yellow-500"}`}
                          >
                  ...
                  return (
                    <div
                      key={seat}
                      className="flex items-center gap-4 rounded-xl border border-dashed border-white/10 bg-white/5 p-3 opacity-50"
                    >
          ...
          <div className="mt-8">
            {room.isQuickMatch ? (
              ...
              <button
                ...
                className="btn-game w-full text-xl py-4 shadow-xl shadow-primary/20 transition-all hover:shadow-primary/40 group relative overflow-hidden"
              >
...
function AnimatedMessage({ chat, players }: { chat: ChatMessage; players: any[] }) {
  ...
  useEffect(() => {
    ...
    const receiverEl = receiverColor
      ? document.getElementById(`base-${receiverColor}`)
  ...
  return (
    <>
      ...
      {isOpen && (
        ...
        >
          <div
            className="bg-card/90 border border-white/10 p-6 rounded-3xl max-w-sm w-full shadow-2xl"
            ...
            {!selectedContent ? (
                ...
                {tab === "phrases" ? (
                  <div className="grid gap-2 max-h-60 overflow-y-auto pr-2">
                    {PHRASES.map((p) => (
                      <button
                        key={p}
                        onClick={() => setSelectedContent({ type: "text", content: p })}
                        className="p-3 bg-white/5 hover:bg-white/10 rounded-xl text-right font-medium transition-colors"
                      >
                        {p}
                      </button>
                    ))}
              ...
              <div className="animate-in fade-in slide-in-from-right-4">
                ...
                <div className="grid gap-2">
                  <button
                    onClick={() => sendChat()}
                    className="p-4 bg-gradient-to-r from-primary to-accent rounded-xl font-bold text-lg hover:brightness-110 transition-all text-white shadow-lg"
                  >
                    الجميع 🌐
                  </button>
                  {opponents.map((p) => (
                    <button
                      key={p.user_id}
                      onClick={() => sendChat(p.user_id)}
                      className="p-3 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl font-medium flex items-center justify-between"
                    >
                      <span style={{ color: p.color }}>
                        {profiles[p.user_id]?.display_name || "Player"}
                      </span>
          ...
          <div className="space-y-3">
            <div className="panel">
              <div className="mb-3 flex items-center justify-between text-sm text-muted-foreground">
                ...
                {!isGameOver && (
                  <span
                    className={`font-bold transition-colors ${timeLeft <= 10 ? "text-destructive animate-pulse" : ""}`}
                  >
              ...
              {displayGame.players.map((p: any, i: number) => {
                ...
                return (
                  <div key={i} className="relative">
                    <div
                      onClick={() => {
                        if (p.userId !== userId) setReactionTarget(i);
                      }}
                      className={
                        p.userId !== userId
                    ...
                    {reactionTarget === i && (
                      <div className="absolute top-full right-0 z-50 mt-2 w-64 p-4 bg-black/90 backdrop-blur-xl rounded-2xl border border-white/20 shadow-2xl animate-in fade-in zoom-in-95">
                          ...
                          <div className="bg-white/5 rounded-lg p-2 col-span-2">
                            <div className="text-sm font-bold text-white">
                              {prof?.stats?.gamesPlayed
                                ? Math.round((prof.stats.wins / prof.stats.gamesPlayed) * 100)
                                : 0}
                              %
                            </div>
                            <div className="text-[10px] text-muted-foreground uppercase tracking-wider">
                              Win Rate ({prof?.stats?.gamesPlayed || 0} matches)
                            </div>
                          </div>
                        </div>

                        <div className="text-xs text-muted-foreground mb-2 uppercase tracking-wider text-center">
                          Send Reaction
                        </div>
                        ...
                        <button
                          ...
                          className="absolute top-2 right-2 text-muted-foreground hover:text-white w-6 h-6 flex items-center justify-center rounded-full bg-white/10"
                        >
                          ✕
                        </button>
                      </div>
          ...
          <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4 animate-in fade-in zoom-in backdrop-blur-sm">
            <div className="panel max-w-md w-full text-center shadow-2xl border border-white/10">
              ...
              <h2 className="text-3xl font-extrabold mb-2 text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">
                {(room.matchCount || 1) >= 5
                  ? "Series Champion!"
                  : `Match ${room.matchCount || 1} Finished!`}
              </h2>
              ...
              <div className="space-y-3 mb-8">
                {leaderboard.map((seat: number, index: number) => {
                  ...
                  return (
                    <div
                      key={seat}
                      className={`flex items-center gap-4 p-3 rounded-xl border border-white/5 bg-black/20 ${index === 0 ? "bg-primary/10 border-primary/30" : ""}`}
                    >
                      <div className={`text-2xl font-black w-8 ${rankColors[index]}`}>
                        ...
                        <div className="text-xs text-muted-foreground flex items-center gap-1">
                          ...
                          {room.coinsEarned?.[p.userId] ? (
                            <span className="ml-1 text-yellow-400 font-bold flex items-center gap-1">
                              • +{room.coinsEarned[p.userId]}{" "}
                              <img src="/coin.png" alt="coins" className="w-3 h-3 inline" />
                            </span>
                          ) : null}
                        </div>
                      </div>
                      ...
                      {userId !== p.userId && !myFriends.has(p.userId || "") && (
                        <button
                          onClick={async () => {
                            ...
                            try {
                              await setDoc(doc(db, `profiles/${userId}/friends`, p.userId), {
                                id: p.userId,
                                addedAt: Date.now(),
                              });
                              await setDoc(doc(db, `profiles/${p.userId}/friends`, userId), {
                                id: userId,
                                addedAt: Date.now(),
                              });
                      ...
                      <div
                        ...
                        style={{ backgroundColor: `var(--ludo-${p.color})` }}
                      />
                    </div>
                  );
                })}
              ...
              <div className="flex gap-3">
                {room.host_id === auth.currentUser?.uid && (room.matchCount || 1) < 5 ? (
                  <button onClick={nextMatch} className="btn-game flex-1 py-4">
                    Start Next Match
                  </button>
                ) : null}
                <button onClick={leave} className="btn-ghost flex-1 py-4">
                  Leave Room
                </button>
              </div>
            </div>
